...
public:
	...
	void print() const;
	void printAllSubsFromDate(const string&) const;
	void clear();
	bool find(const T&);
	void writeToFile();
	void writeToFile(const string&);
	void readFromFile();
	void findAndPrint(int);
	void addToNPos(int);
	void delFromNPos(int);
	~Deque();
...
template<>
void Deque<Sub>::sortInFile(bool order)
{
...
template <typename T>
void Deque<T>::writeToFile(const string& name) {
...
template<typename T>
inline void Deque<T>::findAndPrint(int n)
{
	if (n <= 0 || n > this->sz) {
...
template<typename T>
inline void Deque<T>::addToNPos(int n)
{
	if (n <= 0 || n > this->sz) {
		cout << "there is no such position in deque!\n";
		return;
	}
	
	T temp;
	cout << "please enter subscrider:\n";
	cin >> temp;
	DLNode<T> *newElt = new DLNode<T>(temp);
	...
	while (current != 0) {
		if (i == n) {
			newElt->setPrevPtr(current->getPrevPtr());
			newElt->setNextPtr(current);
			current->getPrevPtr()->setNextPtr(newElt);
			current->setPrevPtr(newElt);
			return;
		}
		else {
			current = current->getNextPtr();
...
template<typename T>
inline void Deque<T>::delFromNPos(int n)
{
	if (n <= 0 || n > this->sz) {
		cout << "there is no such position in deque!\n";
		return;
