	...
	try {
		...
		while (true)
		{
			cout << "enter command to execute:\n"
				...
				<< " 12 - read from file\n"
				<< " 13 - write to file\n"
				<< " 14 - sort info in file\n"
				<< " 15 - print N el-t\n"
				<< " 16 - add el-t to N position\n"
				<< " 17 - delete N el-t\n"
				<< " else - back\n"
				<< "ok, enter: ";
			...
			else if (cmdResponse == 14) {
				cout << "order:\n"
					<< " 1 - ascending\n"
					<< " else - descending\n";

				int order = inputNumber();
				if (order == 1) deque.sortInFile(true);
				else deque.sortInFile(false);
			}
			else if(cmdResponse == 15) {
				cout << "enter n: ";
				int n = inputNumber();
				
				deque.findAndPrint(n);
			}
			else if (cmdResponse == 16) {
				cout << "enter n: ";
				int n = inputNumber();

				deque.addToNPos(n);
			}
			else if (cmdResponse == 17) {
				cout << "enter n: ";
				int n = inputNumber();
